...
class LEDHiveApp():
    ...
    def generate_info():
        return devicehive.DeviceInfo(id='0204eede-2297-11e2-882c-e0cb4eb92129',
                                     ...
                                     network=devicehive.Network(name='VirtualLed Sample Network',
                                                                descr='A DeviceHive network for VirtualLed sample'),
                                     device_class=devicehive.DeviceClass(name='Example Network', version='1.0',
                                                                         is_permanent=False),
                                     equipment=[devicehive.Equipment(name='ExampleEquipment', code='ExampleCode',
                                                                     type='ExampleType')])
    ...
    def on_connected(self):
        def on_save(result):
            self.factory.subscribe(self.DEVICE_INFO.id)

        def on_authenticate(result):
            self.factory.device_save(self.DEVICE_INFO).addCallbacks(on_save, on_failed)
